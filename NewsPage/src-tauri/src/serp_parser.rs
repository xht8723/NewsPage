...
fn resolve_selected_topics(include_topics: &[String], exclude_topics: &[String]) -> Result<Vec<String>, String> {
	let mut selected: Vec<String> = Vec::new();
	let mut include_seen: HashSet<String> = HashSet::new();

	if include_topics.is_empty() {
		selected = SUPPORTED_TOPICS.iter().map(|(name, _)| (*name).to_string()).collect();
	} else {
